...
import com.cmt.vending.server.backendWebSocket.wechatpay.GetWechatQrCodeReq;
...
public class WechatPayment extends Payment {
    ...
    public static final String NAME = "WECHAT_PAY";
    ...
    public void getWechatQrCode() {
        logger.info("[getWechatQrCode]Start to get QR code.");
        ...
        BackendWebSocketClient.sendMessage(GsonUtil.toJson(new GetWechatQrCodeReq(TransactionManager.CURRENT_TX), false, true), 10000);
    }
    @Override
    public void paySuccess() {
        logger.info("[getWechatQrCode-paySuccess]User pay by wechat successfully");
    ...
    public void payFail() {
        if(TransactionManager.CURRENT_TX.getPayStatusCode() != TransactionManager.PAYMENT_STATUS_SUCCESS) {
            BackendWebSocketClient.sendMessage(GsonUtil.toJson(new CancelWechatPaymentReq(TransactionManager.CURRENT_TX), false, true), 10000);
        }
    }
